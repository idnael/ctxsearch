...
import etc
from context import Context, init_languages
...
class Main:
    ...
    def __init__(self):
        ...
        try:
            ...
            print "Please install LangId for language detection support."
        ...
        if visual != None and screen.is_composited():
            ...
            etc.message_dialog(_("Transparency not supported."))
            sys.exit(1)
        ...
        img = Gtk.Image()
        img.set_from_file(os.path.join(etc.DATA_DIR,"floating.png"))
        eventbox.add(img)
        eventbox.show_all()
        ...
        try:
            ...
            etc.message_dialog(_("Need xdotool to run."))
            sys.exit(1)
    ...
    def inic_prop(self, name):
        uname = name.upper()
        value = etc.property(name, getattr(self, "DEFAULT_"+uname))
        setattr(self, uname, value)

    def check_config(self):
        try:
            if etc.check_config():
                if etc.DEBUG: print "Config changed"
