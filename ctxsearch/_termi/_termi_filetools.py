...
import shutil, os, subprocess, urllib, sys, pyinotify, threading, select, pipes
...
def open_files_with_application(application, files):
    ...
    if can_multiple:
        cmd = command + " " + " ".join([ pipes.quote(file) for file in files])
        subprocess.Popen(cmd, shell=True)
    else:
      # If the app can only open one file, I will have to call it many times
      for arg in args:
        cmd = command + " " + pipes.quote(file)
        subprocess.Popen(cmd, shell=True)
