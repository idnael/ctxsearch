...
"""Fields support - Modified by Daniel! <idnael@gmail.com>"""
...
class TerminatorTerm (gtk.VBox):
  ...
  def __init__ (self, terminator, profile = None, command = None, cwd = None):
    ...
    self.file_watcher = FileWatcher(self.file_was_deleted)

    # daniel
    self._vte.set_activate_fields(True)
    self._vte.connect("fields-destroyed", self.on_fields_destroyed)
    self._vte.connect("fields-defined", self.on_fields_defined)

    self.fileops=filetools.FileOperations()

    # daniel
    # internal variable i need to start dragging from the context menu...
    self.menu_drag_release_event = None
    self.show_files_info = False

    # the list of files used to create the last tooltip (a string with the paths separated by newlines)
    self.tooltip_files = ""
    # the widget used in the tooltip
    ...
    #daniel
    # Now we can drag and drop lists of files also!
    dsttargets.append(("text/uri-list",0,0)) 
    ...
    # daniel: added the "| gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_LINK
    self._vte.drag_dest_set(gtk.DEST_DEFAULT_MOTION | gtk.DEST_DEFAULT_HIGHLIGHT |gtk.DEST_DEFAULT_DROP ,dsttargets, gtk.gdk.ACTION_MOVE| gtk.gdk.ACTION_COPY| gtk.gdk.ACTION_LINK  )
    ...
    self._vte.connect("drag-drop", self.on_drag_drop) # estava drag_drop...
    self._vte.connect("drag-end", self.on_drag_end)

    # daniel
    self._vte.set_property("has-tooltip", True)
    self._vte.connect("query-tooltip", self.on_query_tooltip)
    #self._vte.connect("motion-notify-event", self.on_mouse_motion)
  ...
  def on_drag_data_get(self,widget, drag_context, selection_data, info, time, data):
    ...
    if "vte" in drag_context.targets:
      # its the internal dnd of terminator terms:
      selection_data.set("vte",info, str(data.terminator.term_list.index (self)))

    else:
      # this variable is defined when we do drag-begin
      if self.dragged_files:

        # this is a file drag operation activated from the context menu!
        #print "ON_DRAG_DATA_GET files!!!"

        # convert the files into uris
        uris = filetools.paths_to_uris(self.dragged_files)
...
text/plain
    ...
    # daniel - ignore also "text/uri-list"
    if 'text/plain' in drag_context.targets or "text/uri-list" in drag_context.targets:
  ...
  def on_drag_end(self, widget, drag_context):
    ...
    # probably some files don't exist anymore:
    self.check_non_existing_fields()


  # daniel
  # isto nunca era chamado!! agora ja eh
  ...
  # daniel
  # called when the user drop files in any place in the terminal
  def drop_files(self, drag_context, x, y, selection_data, info, time, data):

    files = filetools.uris_to_paths(selection_data.get_uris())

    # the position of the event in the terminal
    (col,row) = self._vte.translate_xy(x,y)

    field_id=self._vte.get_field_at(col,row)
    if field_id != -1:

      destination_folder = self.file_watcher.get_file(field_id)

      if not os.path.isdir(destination_folder):
        print "nao eh dir"
        # como fazer para rejeitar o drop, para dar umsinal ao user?
        # nao... drag_context.drop_reply(False, time)        

      else:
        #print ("copiar" if is_copy else "mover"),"para",destination_folder

        menu = gtk.Menu ()

        item = gtk.MenuItem("To "+os.path.basename(destination_folder))
        item.set_sensitive(False)
        menu.append(item)

        item_text = filetools.action_name(drag_context.action)+ " "+filetools.resume(files)
        
        item = gtk.MenuItem(item_text)
        item.connect('activate', lambda menuitem: (self.fileops.copy_or_move(destination_folder, drag_context.action, files), self.check_non_existing_fields()) )
        menu.append(item)

        menu.show_all()
        menu.popup(None, None, None, 1, time) # button 1?
    ...
    #    term._vte.feed_child(txt)
      ...
      (cursor_col,cursor_row) = self._vte.get_cursor_position()

      # to paste as text, drop in the cursor!

      if row == cursor_row and col >= cursor_col:
        # paste do texto:
        txt="".join( ["'"+file+"' " for file in files ])
  ...
  #daniel...
  on_drag_data_received_last_time = -1
  ...
  def on_drag_data_received(self, widget, drag_context, x, y, selection_data, info, time, data):
    ...
    # I also receive this... have to ignore then
    if selection_data.type == 'text/plain':
      return

    if selection_data.type == 'text/uri-list': ###'text/plain':
      # if I drag files from one terminal to other or the same terminal,
      # the method "on_drag_data_received" is called twice...
      # So I do this test to reject the second one:
      if time_now() - self.on_drag_data_received_last_time < 1: return
      self.on_drag_data_received_last_time = time_now()

      self.drop_files(drag_context,x,y,selection_data,info,time,data)
...
#      delbind = vte.ERASE_AUTO
    ...
    if hasattr (self._vte, "set_cursor_shape"):
      self._vte.set_cursor_shape (getattr (vtepp, "CURSOR_SHAPE_" + self.conf.cursor_shape.upper ()))
  ...
  def file_was_deleted(self, id):
    print "file_was_deleted", id
    self._vte.cancel_field(id, True)


  def on_fields_destroyed(self, term): ###,field_id):
    print "on_fields_destroyed"
    ids = self._vte.fields_destroyed()
    if not ids: return
    print ids
    for id in ids:
      self.file_watcher.remove_watch(id)

  def on_fields_defined(self, term): ##, field_id):
    print "on_fields_defined"
    ids = self._vte.fields_defined()
    if not ids: return
    print ids
    for id in ids:
      # converts in a normalized file path
      file = os.path.abspath(self._vte.get_field_metadata(id))
      
      # Return True if file refers to an existing file. 
      # Returns True for broken symbolic links. 
      if not os.path.lexists(file):
        # pode ser por exemplo o target de um sym link partido
        # cancela, mas sem riscar!
        print "Field invalido ("+file+")"
        self._vte.cancel_field(id,False)
    
      else:
    
        self.file_watcher.add_watch(id, file)
  ...
  def create_popup_menu (self, widget, event = None):
    ...
    if event:
      ...
      # daniel
      files = self.get_selected_files(event.x, event.y)

      # if there is no files, see if there is a url
      if not files:
        (col,row) = self._vte.translate_xy(int(event.x), int(event.y))
        url = self._vte.match_check (col, row)
    ...
    if files:
      ...
      item = gtk.MenuItem(filetools.resume(files)+":") # only a comment
      item.set_sensitive(False)
      menu.append(item)

      def_app = filetools.get_default_application_for_files(files)
      if def_app:
        item = gtk.MenuItem(_("Open with") + " " + def_app[1])
        item.connect('activate', self.open_with_app, def_app, files)
        menu.append(item)

      all_apps = filetools.get_applications_for_files(files)
      other_apps = [app for app in all_apps if app[0] != def_app[0] ]
      ...
      if other_apps:
        ...
        for app in other_apps:
          subitem = gtk.MenuItem(app[1])
          subitem.connect("activate", self.open_with_app, app, files)
          submenu.append(subitem)

      elif not def_app:
        item = gtk.MenuItem(_("Open with default applications"))
        item.connect('activate', self.open_with_default_apps, files)
        menu.append(item)
        

      item = gtk.MenuItem(_("Drag"))
      item.connect('activate', self.on_drag_menuitem_activate, files)
      item.connect("button-release-event", self.on_drag_menuitem_release, files)
      #item.connect("button-press-event", self.on_menu_drag_press, files)
      menu.append(item)

      item = gtk.MenuItem("Copy")
      item.connect('activate', lambda menu_item: self.fileops.copy_to_clipboard(files))
      menu.append(item)

      item = gtk.MenuItem("Cut")
      item.connect('activate', lambda menu_item: self.fileops.copy_to_clipboard(files, True))
      menu.append(item)

      if len(files) == 1 and os.path.isdir(files[0]):
        item = gtk.MenuItem("Paste here")
        item.connect('activate', lambda menu_item: self.fileops.paste_from_clipboard(files[0]))
        self.fileops.clipboard_files_available(lambda result, item: item.set_sensitive(result), item)
        menu.append(item)
        
      item = gtk.MenuItem(_("Delete"))
      item.connect('activate', lambda menu_item: self.fileops.delete_confirm(files))
      menu.append(item)
    ...
    # daniel - will change the text to "copy text" to diferentiate from the "copy files" operation 
    item = gtk.MenuItem ("Copy text")
    #item = gtk.ImageMenuItem (gtk.STOCK_COPY)
    item.connect ("activate", lambda menu_item: self._vte.copy_clipboard ())
    item.set_sensitive (self._vte.get_has_selection ())
    menu.append (item)

    item = gtk.MenuItem ("Paste text")
    ...
    item = gtk.CheckMenuItem(_("Show file properties"))
    item.set_active(self.show_files_info)
    item.connect('toggled', self.do_ShowInfo_toggle)
    menu.append(item)
  ...
  def on_drag_menuitem_activate(self, menuitem, files):
    #print "on_menu_drag"
    event = self.menu_drag_release_event

    # there are two ways we get here
    # 1. user press button3 to open the context menu, and imediatly releases the button. The menu will remain open and the user will go to the menuitem and press again and release, and this will activate the menuitem. In this case the event will have button=1
    # 2. the user in a single click opens the menu and selects one option. In this case, the event will have button=3

    # That's why I always pass 1. This is the button that the user will have to press to drop

    self._vte.drag_begin(  [("text/uri-list",0,0)], gtk.gdk.ACTION_MOVE | gtk.gdk.ACTION_COPY, 1, event)

    self.dragged_files = files

  def on_drag_menuitem_release(self,menuitem, event, files):
    #print "on_menu_drag_release"
    self.menu_drag_release_event = event


  # this detects fields that have been deleted, since
  # pyinotify can't detect when a file is moved...
  def check_non_existing_fields(self):
    pass
    # passar para o file_watcher...

    #for field_id in self.fields:
    #  file = self.fields[field_id]
    #  if not os.path.lexists(file):
    #    self._vte.cancel_field(field_id, True)




  def get_selected_files(self, x, y):
      # the position in the terminal
      (col,row) = self._vte.translate_xy(int(x), int(y))

      if self._vte.inside_selection(col,row):
        # the operation will apply to all the selected fields
        field_ids=self._vte.get_selected_fields()
      ...
      for field_id in field_ids:
        file = self.file_watcher.get_file(field_id)

        # if a path doesn't exist, cancel it
        # Can be one of the cases not detected by pyinotify
  ...
  def on_query_tooltip(self, widget, x, y, keyboard_mode, tooltip):
    if not self.show_files_info:
      return False

    files = self.get_selected_files(x, y)

    if len(files) == 0:
      return False

    files_as_str = "\n".join(files)
    if files_as_str != self.tooltip_files:
      self.tooltip_files = files_as_str
      self.tooltip_widget = filetools.details(files)
  ...
  def do_ShowInfo_toggle(self, mi):
    print "toggleShowInfo"
    self.show_files_info = mi.get_active() ##not self.show_files_info

  def open_with_app(self, menu_item, app, files):
    print "open_with_app",app[2],files
    filetools.open_files_with_application(app, files)

  def open_with_default_apps(self, menu_item, files):
    print "open_with_default_apps",files

    errors = ""

    for file in files:
      app = filetools.get_default_application_for_files([file])
      if app:
        filetools.open_files_with_application(app, [file])
      else:
        errors += os.path.basename(file) + "\n"
    ...
    if errors:
      ...
      vbox.add(gtk.Label(_("No application to open these files:")))

      text = gtk.TextView()
      text.set_editable(False)
      text.get_buffer().set_text(errors)
